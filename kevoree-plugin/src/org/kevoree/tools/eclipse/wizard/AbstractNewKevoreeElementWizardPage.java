 ...
 */
public abstract class AbstractNewKevoreeElementWizardPage extends NewTypeWizardPage {

	protected static final int COLS = 4;

	private IResource resource;

	private FieldInitializerUtil util =new FieldInitializerUtil();
	...
	public AbstractNewKevoreeElementWizardPage(int typeKind, String title, String extension) {
		...
		IRunnableWithProgress op = new WorkspaceModifyOperation() {
			...
			protected void execute(IProgressMonitor monitor) throws CoreException, InvocationTargetException,
				...
				try {
					...
					IFile xtendFile = ((IFolder) res).getFile(getTypeName() + extension); //$NON-NLS-1$
					//URI uri = storage2UriMapper.getUri(xtendFile);
					size[0] = createXtendElement(monitor, xtendFile, "\t","\n");
				} catch (Exception e) {
	...
	protected IStatus typeNameChanged() {
		...
		if (packageFragment != null) {
			...
			if (resource instanceof IFolder) {
				...
				if (folder.getFile(getTypeName() + extension).exists()) { //$NON-NLS-1$
